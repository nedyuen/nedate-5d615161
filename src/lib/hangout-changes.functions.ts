...
function fmtWhen(start: string | null | undefined, end: string | null | undefined) {
  ...
  const base = `${s.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })} · ${s.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}`;
  if (!end) return base;
  return `${base} – ${new Date(end).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}`;
}
